...
*** dialog 本身的 UI 延後到第一次真的要開啟才建，但 `_on_auto_close_
error()` 這個全域 toast 訂閱不能跟著延後 ***：跟 `web_quote_board_
page.py` 同一個理由(見該檔開頭/`app/services/lazy_ui.py` 的完整說
明)，部位清單/分組表頭/自動平倉規則列這些 UI 建構本身有實際成本，不需
要在 `main.py::index()` 連線成功的當下就先建完。但自動平倉錯誤是背景
事件驅動的全域提示，使用者可能根本還沒開過「部位」這個對話框就先遇到
錯誤——這個訂閱要立刻註冊，不能等使用者第一次點開對話框才開始接收。
"""
import asyncio
import datetime
...
def build(position_manager: PositionManager, auto_close_manager: AutoCloseManager) -> Callable:
    ...
    # 立刻註冊，不放進 _build_dialog()——理由見檔案開頭的說明。
