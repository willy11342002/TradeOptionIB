...
*** dialog 本身的 UI(AG Grid＋技術分析頁籤)延後到第一次真的要開啟才建
***：實測抓到真實案例——`main.py::index()` 連線成功後緊接著同步組裝這
個 dialog(連同下單面板/委託簿/部位)，AG Grid＋技術分析頁籤這些 UI 建
構本身就要吃掉可觀的時間，加總很容易超過 NiceGUI `@ui.page` 預設 3 秒
的 `response_timeout`，讓整頁被砍成 500(參見 `app/services/lazy_ui.py`
開頭的完整說明)。`quote_client`/`_get_contract` 這兩個是資料層、其他頁
面(main.py 的 PositionManager／下單面板)在 dialog 開啟前就需要用到，所
以留在 `_build_dialog()` 外面立刻建好；真正畫 UI 的部分包進
`_build_dialog()`，靠 `lazy_open()` 包裝成「第一次呼叫才真的建」的
`open_dialog`。
...
def build(ib_client: IBClient, on_leg_selected: Optional[Callable] = None):
    ...
    def _get_contract(strike: float, is_call: bool):
        # 只讀 state，不碰任何 UI 元件——刻意留在 _build_dialog() 外面，
        # 下單面板在報價盤 dialog 第一次被打開之前就可能需要呼叫這個(見
        # 檔案開頭「dialog 本身的 UI 延後到第一次真的要開啟才建」的說
        # 明)，這時候 state["contracts_by_strike"] 還是空的，直接回傳
        # None，跟「這個到期日沒有這個履約價」是同一種結果，呼叫端本來
        # 就要處理。
    ...
    open_dialog = lazy_open(_build_dialog)
